...
if sys.version_info < python_version:
    print("It looks like you are using a python version older then python2.5.0\n"
...
FEED_SOFT_ERROR_WARNING = 'Soft error "%s" in "%s" , doesnt seem to be major - moving on'
...
def _nettest(fn):
    ...
    def _decorate(lastchecked, oldentries, uri):
        ...
        if newfeedobj.bozo == 0:
            ...
            print FEED_HARD_ERROR_WARNING % (uri, newfeedobj.bozo_exception.message)
            return oldentries
        else:
            print FEED_SOFT_ERROR_WARNING % (newfeedobj.bozo_exception.message, uri)
            entries = fn(lastchecked, oldentries, newfeedobj.entries)
            return entries
    return _decorate
...
def _updatefeed(lastchecked, oldentries, newentries):
    """Currently it can be used to merge two feeds using the check_if_new function
    
    You give this function a feeds LastChecked, Entries, and a new set of entries
...
class FeedDatabase(object):
    ...
    * echo*: True/False, if True sqlalchemy will act vocally
    """
    def __init__(self, dblocation='sqlite:///:memory:', echo= True):
        """
        Arguments        
...
if __name__ == '__main__':
    sampledb = FeedDatabase(echo=False)
    samplefeedurls = ['http://planet.ubuntu.com/rss20.xml',
                      'http://planet.fedoraproject.org/rss20.xml',
                      'http://planet.gnome.org/rss20.xml',
                      'http://www.groklaw.net/backend/GrokLaw.rdf']
    for feed in samplefeedurls:
        sampledb.save_new_feed(BuildFeed(feed))
    feedobjects =  sampledb.get_feed_objects()
    sampledb.update_feeds(samplefeedurls)
    print ''
    for item in feedobjects:
        print item
