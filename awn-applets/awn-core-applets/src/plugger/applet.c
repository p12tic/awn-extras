...
volume_add(Plugger *app, GnomeVFSVolume *volume)
{
  ...
  gchar *name = NULL;
  if(type != GNOME_VFS_DEVICE_TYPE_HARDDRIVE &&
        type != GNOME_VFS_DEVICE_TYPE_NFS &&
        type != GNOME_VFS_DEVICE_TYPE_AUTOFS &&
        type != GNOME_VFS_DEVICE_TYPE_SMB &&
        type != GNOME_VFS_DEVICE_TYPE_WINDOWS &&
        type != GNOME_VFS_DEVICE_TYPE_LOOPBACK &&
        type != GNOME_VFS_DEVICE_TYPE_NETWORK){
