...
from qfeed import QueryFeed, NAME, URL, TYPE, TIME_FORMAT, IMG_INDEX, ALT_TEXT
...
class ComicsAdder:
	...
	def __init__(self):
		...
		self.model = gtk.ListStore(gobject.TYPE_UINT, gtk.gdk.Pixbuf)
	...
	def on_add_assistant_apply(self, widget):
		...
		if self.img_index > 1:
			settings[IMG_INDEX] = self.img_index
			settings.comments[IMG_INDEX] = \
				'The index of the image in the HTML-code'
		
		for i, alt in self.feed.alt_texts:
			if i == self.img_index and alt:
				settings[ALT_TEXT] = alt
				settings.comments[ALT_TEXT] = 'Disregard any comics where ' + \
					'the alt text of the img element does not match "%s"' % alt
	...
	def on_add_assistant_prepare(self, widget, page):
		if page is self.xml.get_widget('url_page'):
			...
			self.feed.connect('updated', self.on_feed_updated)
			self.feed.update()
		
		elif page is self.xml.get_widget('image_page'):
			# Download images for the image list
			self.model.clear()
			for (index, url) in self.feed.images:
				iterator = self.model.append((index, None))
	...
	def on_url_entry_changed(self, widget):
		...
		self.assistant.set_page_complete(self.xml.get_widget('url_page'),
			len(text) > 7 and text.find('http://') > -1)
	...
	def on_image_list_selection_changed(self, widget):
		...
		if len(selection) == 1:
			self.assistant.set_page_complete(self.xml.get_widget('image_page'),
				True)
			self.img_index = self.model.get_value(
	...
	def on_time_entry_changed(self, widget):
		try:
			t = strptime(self.time_sample, widget.get_text(), True)
			self.assistant.set_page_complete(self.xml.get_widget('time_page'),
				True)
		except:
			self.assistant.set_page_complete(self.xml.get_widget('time_page'),
				False)
	...
	def on_feed_updated(self, feed, result):
		...
		if len(feed.images) > 1:
			...
			if len(feed.images) == 1:
				self.img_index = feed.images[0][0] + 1
	...
	def on_image_download_completed(self, o, code, iterator):
		...
		if result != Downloader.DOWNLOAD_OK:
