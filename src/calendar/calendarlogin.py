...
class CalendarLogin(gtk.Window):
	...
	def ok_button(self, widget, event):
		...
		if self.save_password_checkbox.get_active() == True:
			...
			dialog = gtk.MessageDialog(parent=None, flags=gtk.DIALOG_MODAL, buttons=gtk.BUTTONS_YES_NO, type=gtk.MESSAGE_WARNING, message_format=warning)
			response = dialog.run()
		...
		self.applet.gconf_client.set_string(self.applet.gconf_path + "/password", saved_password)		
		self.hide()
