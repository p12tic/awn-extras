...
class StacksGuiDialog():
    ...
    def __init__ (self, applet):
        ...
        applet.connect("stacks-store-changed", self._stacks_store_changed_cb)
    ...
    def _stacks_store_changed_cb(self, widget, store, iter):
        self.store = store
    ...
    def _dialog_restructure_cb(self, widget, page=0):
        ...
        while iter:
            ...
            if not button:
                button = self._item_created_cb(None, iter)
            t = button.get_parent()
            if t:
                t.remove(button)
    ...
    def dialog_focus_out(self, widget, event):
        return self._stacks_gui_hide_cb(widget)
